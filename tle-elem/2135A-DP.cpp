...
void solve(int _){
    ...
    vector<int> dp(n+1, 0);
    // ekhane dp[i] = ekhon porjonto max seq er len koto
    ...
    for(int i = 1; i <= n; i++){
    	...
    	if(len >= curr){
    		// x hoitese dan dik theke (curr) element er curr tomo occurence er index (karon curr shonkhok mal lagbe)
    		// karon amra jodi ei curr elem ta nei, amader lagbe sheitar ager ghorer dp er value.
    		int x = pos[curr][len-curr];
    		dp[i] = max(dp[i-1], dp[x-1] + curr);
    	} else {
    		dp[i] = dp[i-1];
    	}
    }
